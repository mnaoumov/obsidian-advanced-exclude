...
This vault ships an `Archive/` folder with two notes - [[Archive/Old note]] and
[[Archive/Draft]] - as the exclusion target.

## Before you start

1. In the Files pane, confirm you can see the `Archive/` folder with its two notes.
2. Open [[Shared/Topic]] and show its **Backlinks** pane (right-click the tab ->
...
- The backlink from `Old note` disappears from [[Shared/Topic]]'s **Backlinks** pane.
- A global **Search** for `Draft` no longer returns the archived note.
- The excluded notes drop out of **Graph view**.

The trailing slash in `Archive/` matters: it targets a folder. See [[04 Settings]]
for the setting key this text area writes to, and [[02 Whitelist with negation]]
